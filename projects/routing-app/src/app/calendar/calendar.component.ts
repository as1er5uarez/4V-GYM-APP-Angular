...
export class CalendarComponent {
  ...
  daySelected(day: number, month: string, year: string): void {
    ...
    if (h3 != null) {
      ...
      this.currentDate.setHours(0, 0, 0, 0); // Ajustar a medianoche
      this.showActivities(day, month, year);
    }
  }
  ...
  showActivities(day: number, month: string, year: string): void {
    ...
    for (let actividad of this.actividades) {
      let actividadDate = new Date(actividad.date);
      actividadDate.setHours(0, 0, 0, 0); // Ajustar a medianoche
      if (actividadDate.getTime() === date.getTime()) {
  ...
  onSubmit() {
    const newActivityDate = new Date(this.currentDate.getTime());
    if (this.actividadForm.valid) {
      ...
      this.showActivities(newActivityDate.getDate(), newActivityDate.getMonth().toString(), newActivityDate.getFullYear().toString());
      this.refreshView();
